...
artifact_root="${DIALSHOT_ARTIFACT_DIR:-$repo_root/build/ci-artifacts}"
...
grep -q "TARGETED_DEVICE_FAMILY = 1;" DialShot.xcodeproj/project.pbxproj \
  || { echo "::error::TARGETED_DEVICE_FAMILY = 1 missing from project"; exit 1; }
grep -E "TARGETED_DEVICE_FAMILY = (1,2|2);" DialShot.xcodeproj/project.pbxproj \
...
bad=$(grep -E "TARGETED_DEVICE_FAMILY = [^1;]" DialShot.xcodeproj/project.pbxproj || true)
if [ -n "$bad" ]; then
  echo "::error::Non-iPhone TARGETED_DEVICE_FAMILY values found:"
  echo "$bad"
  exit 1
fi
count=$(grep -c "TARGETED_DEVICE_FAMILY = 1;" DialShot.xcodeproj/project.pbxproj)
...
app_plist="$derived_data/Build/Products/Debug-iphonesimulator/DialShot.app/Info.plist"
if [[ ! -f "$app_plist" ]]; then
...
if bundle_id != "com.infinityball.dialshot":
    raise SystemExit(
        "Built app CFBundleIdentifier must be com.infinityball.dialshot, "
        f"observed {bundle_id!r}"
    )
print("Verified built app UIDeviceFamily == [1] (iPhone only)")
print("Verified built app CFBundleIdentifier == com.infinityball.dialshot")
PYTHON
